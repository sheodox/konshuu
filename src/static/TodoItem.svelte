...
{#if !todo.completed || !$hideCompleted}
	<li class="todo-item f-row align-items-start px-1" draggable="true" on:dragstart={dragStart} class:muted={todo.completed}>
		<Checkbox
...
{#if showReschedule}
	<Reschedule bind:visible={showReschedule} {listType} {calendarDate} on:reschedule={rescheduleTodo} />
{/if}
...
<script>
	...
	async function rescheduleTodo(e) {
		...
		await fetch(`/todo/${todo.id}/reschedule/${(getRescheduleDestination(e.detail).serialize())}`);
		await updateWeek();
	}
