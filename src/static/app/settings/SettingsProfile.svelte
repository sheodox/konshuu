<form on:submit|preventDefault={saveProfile} class="f-column gap-3">
	<TextInput bind:value={email} id="settings-email">Email</TextInput>
	<TextInput bind:value={firstName} id="settings-first-name">First Name</TextInput>
	<TextInput bind:value={lastName} id="settings-last-name">Last Name</TextInput>
	<button class="save" disabled={!profileSaveable || saving}>Save</button>
</form>

<script lang="ts">
	import { createAutoExpireToast, TextInput } from 'sheodox-ui';
	import { appBootstrap } from '../stores/app';

	let user = appBootstrap.user,
		email = user.email,
