...
<button on:click={setToNow} type="button">Reset to now</button>
...
<script lang="ts">
	...
	let { date, time } = getInitial(data.countdownEnd);
	...
	function getInitial(date: Date) {
		const hasValidDate = date instanceof Date;
		return {
	...
	function setToNow() {
		const newValues = getInitial(new Date());
		date = newValues.date;
		time = newValues.time;
	}
</script>
